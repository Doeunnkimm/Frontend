...
it('"Wish Mart" 텍스트 로고을 클릭할 경우 "/" 경로로 navigate가 호출된다.', async () => {
  ...
  await user.click(screen.getByText('Wish Mart')); // 클릭 이벤트를 시뮬레이션
...
describe('로그인이 된 경우', () => {
  ...
  beforeEach(() => {
    // 기존 handlers.js 응답 → use 함수 내에서 응답을 기준으로 테스트 실행
    ...
    mockUseUserStore({ isLogin: true }); // 로그인된 것처럼 설정
    ...
    mockUseCartStore({ cart }); // 장바구니에 상품이 담긴 것처럼 설정
  });
